...
async function findAllWithChildren() {
  ...
  const [planos, manuts, abastecs] = await Promise.all([
    ...
    db.getMany(`SELECT * FROM veiculo_abastecimentos WHERE veiculo_id IN (${ph}) ORDER BY data DESC, created_at DESC`, ids),
  ]);

  const planosByVeic = new Map();
  const manutsByVeic = new Map();
  const abastecsByVeic = new Map();
  ...
  for (const a of abastecs) {
    if (!abastecsByVeic.has(a.veiculoId)) abastecsByVeic.set(a.veiculoId, []);
    abastecsByVeic.get(a.veiculoId).push(a);
  }
  ...
  return veiculos.map((v) => ({
    ...
    abastecimentos: abastecsByVeic.get(v.id) || [],
  }));
}
...
async function findByIdWithChildren(id) {
  ...
  const [planos, manutencoes, abastecimentos] = await Promise.all([
    ...
    db.getMany(`SELECT * FROM veiculo_abastecimentos WHERE veiculo_id = $1 ORDER BY data DESC, created_at DESC`, [id]),
  ]);
  return { ...veiculo, planos, manutencoes, abastecimentos };
}
