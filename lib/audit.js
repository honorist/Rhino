 ...
 * @type {string[]}
 */
const SKIP_PREFIXES = ['/api/notificacoes'];
...
function sanitizeBody(body, _depth = 0) {
  ...
  if (Array.isArray(body)) {
    return body
      .slice(0, 100)
      .map((v) => (v && typeof v === 'object' ? sanitizeBody(v, _depth + 1) : v));
  }
  const out = {};
...
async function log({ req, body, status, durationMs, requestId }) {
  try {
    ...
    const label =
      req._auditEntityLabel || _resolveLabel(before) || _resolveLabel(sanitizeBody(body)) || null;
...
/**
 * Lista eventos do audit log com filtros opcionais.
 *
 * Paginação: por CURSOR quando `afterTs`+`afterId` são passados (P1-4 da DB
 * review — seek via índice em `(ts, id)` em vez de OFFSET, que numa tabela
 * grande obriga o Postgres a escanear e descartar todas as linhas anteriores
 * a cada página). Sem cursor, cai no OFFSET de sempre — é só a 1ª página, ou
 * um caller que ainda não migrou. O cursor é (ts, id) da ÚLTIMA linha da
 * página anterior; `id` desempata quando dois eventos têm o mesmo `ts`
 * (timestamps podem colidir; `id` bigserial é uma ordem total estável).
 *
 * @param {{ user?: string, entity?: string, action?: string, from?: string, to?: string, limit?: number, offset?: number, afterTs?: string, afterId?: number|string }} [filters]
...
async function listEvents({
  ...
  if (entity) {
    whereVals.push(entity);
    conds.push(`entity = $${whereVals.length}`);
  }
  if (action) {
    whereVals.push(action);
    conds.push(`action = $${whereVals.length}`);
  }
  if (from) {
    whereVals.push(from);
    conds.push(`ts >= $${whereVals.length}`);
  }
  if (to) {
    whereVals.push(to);
    conds.push(`ts <= $${whereVals.length}`);
  ...
  if (errorsOnly) {
    conds.push(`status >= 400`);
  }
  // "total" reflete só os FILTROS (não o cursor) — é "quantos no total", não
  // "quantos depois desta página". Snapshot antes de acrescentar o cursor.
  const where = conds.length ? `WHERE ${conds.join(' AND ')}` : '';
  const total = await db.getOne(`SELECT COUNT(*)::int AS n FROM audit_log ${where}`, whereVals);

  const rowConds = [...conds];
  const rowVals = [...whereVals];
  const usaCursor = afterTs != null && afterId != null;
  if (usaCursor) {
    rowVals.push(afterTs);
    const tsIdx = rowVals.length;
    rowVals.push(afterId);
    rowConds.push(`(ts, id) < ($${tsIdx}, $${rowVals.length})`);
  }
  const rowWhere = rowConds.length ? `WHERE ${rowConds.join(' AND ')}` : '';
  const rowQuery = usaCursor
    ? `SELECT id, ts, user_id, user_email, ip, method, path, entity, entity_id, action, status, duration_ms, body, request_id, before_state, entity_label
       FROM audit_log ${rowWhere} ORDER BY ts DESC, id DESC LIMIT $${rowVals.length + 1}`
    : `SELECT id, ts, user_id, user_email, ip, method, path, entity, entity_id, action, status, duration_ms, body, request_id, before_state, entity_label
       FROM audit_log ${rowWhere} ORDER BY ts DESC, id DESC LIMIT $${rowVals.length + 1} OFFSET $${rowVals.length + 2}`;
  const rows = await db.getMany(rowQuery, usaCursor ? [...rowVals, limit] : [...rowVals, limit, offset]);
...
/**
 * Retenção de auditoria (steering §12: "retenção de auditoria/IP com purga
 * automática" — promessa sem implementação até a varredura 2026-09-08).
 *
 * MASCARA em vez de apagar a linha: além da janela de retenção, zera `ip`,
 * `body` e `before_state` (o payload mais sensível/volumoso), preservando
 * `entity`/`entity_id`/`action`/`user_email`/`ts` — o traço "quem fez o quê,
 * quando" continua disponível pra auditoria de longo prazo, só o detalhe
 * fino (IP, diff completo) é que tem vida útil curta. Mesmo padrão de
 * `lib/pg-rate-limit.js#cleanup`.
 *
 * @param {number} [retentionDays=180]
 * @returns {Promise<number>}  Linhas mascaradas.
 */
async function purgeOldDetails(retentionDays = 180) {
  const result = await db.query(
    `UPDATE audit_log
        SET ip = NULL, body = NULL, before_state = NULL
      WHERE ts < NOW() - ($1 || ' days')::interval
        AND (ip IS NOT NULL OR body IS NOT NULL OR before_state IS NOT NULL)`,
    [String(retentionDays)]
  );
  return result.rowCount || 0;
}

module.exports = { log, listEvents, detectEntity, sanitizeBody, maskSensitive, purgeOldDetails };
