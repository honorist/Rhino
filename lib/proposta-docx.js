...
async function gerarDocx(p, opts = {}) {
    ...
    try {
      ...
      if (bgPath) {
        ...
        bgChildren.push(new ImageRun({
          data: bgData,
          type: bgType,
          // A4 em pixels @ 96 dpi: 794×1123 (não escala perfeito mas funciona);
          // a lib docx converte transformation.width/height → EMU automaticamente.
  ...
  const doc = new Document({
    ...
    sections: [{
      properties: {
        page: {
          // Left 1.9" = ~135pt = ~4.8cm: conteúdo deslocado pra direita, longe
          // da logo/decoração vermelha do timbrado no lado esquerdo do fundo.
          margin: {
            top:    convertInchesToTwip(1.2),
            bottom: convertInchesToTwip(0.9),
            left:   convertInchesToTwip(1.9),
