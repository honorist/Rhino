...
const { richTextToDocxParagraphs } = require('./rich-text-to-docx');

// Campo de texto rico "de verdade" (editor Jodit) vs. texto legado — mesma
// detecção usada em lib/proposta-html.js#renderRichText, pra propostas salvas
// antes desta mudança continuarem renderizando exatamente como antes.
...
async function gerarDocx(p, opts = {}) {
  ...
  // Anexos já carregados com binário (mesma fonte usada pela galeria de
  // imagens abaixo) — usado pra resolver <img src="/api/propostas/:id/anexos/:id">
  // embutida em campos de texto rico.
  const anexoMap = {};
  (skipImages ? [] : (p.anexos || [])).forEach(a => { if (a.data) anexoMap[a.id] = { data: a.data, mimeType: a.mimeType }; });
  ...
  if (HTML_TAG_RE.test(saudacaoVal)) {
    body.push(...richTextToDocxParagraphs(saudacaoVal, { anexoMap, destaque: true }));
  } else {
    body.push(p_(txtDestaque(saudacaoVal), { alignment: AlignmentType.JUSTIFIED, after: 200 }));
  }
  ...
  if (p.objetivo) {
    ...
    if (HTML_TAG_RE.test(p.objetivo)) {
      body.push(...richTextToDocxParagraphs(p.objetivo, { anexoMap, destaque: true }));
    } else {
  ...
  if (inclusos.length === 0) {
    ...
    inclusos.forEach(it => {
      ...
      if (HTML_TAG_RE.test(texto)) body.push(...richTextToDocxParagraphs(texto, { anexoMap, destaque: false }));
      else body.push(bullet(texto));
    });
  }
  ...
  const renderClausulas = (lista) => {
    ...
    lista.forEach(c => {
      ...
      const texto = c.texto || '';
      if (HTML_TAG_RE.test(texto)) {
        body.push(...richTextToDocxParagraphs(texto, { anexoMap, destaque: true }));
      } else {
        body.push(p_(txtDestaque(texto), { alignment: AlignmentType.JUSTIFIED, after: 120 }));
      }
    });
  };
  ...
  if (p.observacoes) {
    ...
    if (HTML_TAG_RE.test(p.observacoes)) {
      body.push(...richTextToDocxParagraphs(p.observacoes, { anexoMap, destaque: true }));
    } else {
