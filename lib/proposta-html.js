...
${(p.metadata && p.metadata.apresentacao) ? `
  <h2>Apresentação</h2>
  ${textoParaBullets(p.metadata.apresentacao)}
` : ''}

${(p.metadata && p.metadata.casesSucesso) ? `
  <h2>Cases de Sucesso Recentes</h2>
  ${textoParaBullets(p.metadata.casesSucesso)}
` : ''}

${(p.metadata && p.metadata.segurancaSaude) ? `
  <h2>Segurança e Saúde</h2>
  ${textoParaBullets(p.metadata.segurancaSaude)}
` : ''}
