...
const { sanitizeRichText } = require('./rich-text-sanitize');
...
/**
 * Renderiza texto com 'Contratada' e 'Contratante' em <strong>NEGRITO MAIÚSCULO</strong>.
 * Já aplica esc() em cada segmento.
 */
function escComDestaque(s) {
  const segs = cfg.segmentarComDestaque(s);
  return segs.map(seg => seg.highlight
    ? `<strong>${esc(seg.text)}</strong>`
    : esc(seg.text)
  ).join('');
}
...
/**
 * Igual a escComDestaque, mas sem chamar esc() nos segmentos — usado quando o
 * texto já é HTML de verdade (editor rico) e vai passar por sanitizeRichText
 * em seguida, que faz a validação/limpeza da marcação inteira de uma vez.
 */
function destacarSemEscapar(s) {
  const segs = cfg.segmentarComDestaque(s);
  return segs.map(seg => seg.highlight ? `<strong>${seg.text}</strong>` : seg.text).join('');
}

/**
 * Renderiza um campo de texto rico (objetivo/saudação/observações/itens de
 * escopo/obrigações — os únicos campos com editor Jodit no frontend).
 *
 * Texto legado sem nenhuma tag HTML (propostas salvas antes desta mudança)
 * continua com o EXATO comportamento de antes: escapado, opcionalmente com o
 * destaque automático de Contratada/Contratante, envolto em <p>. Texto novo
 * (com tags) passa pelo allowlist de sanitizeRichText — o destaque automático,
 * quando aplicável, é inserido ANTES da sanitização para conviver com a
 * formatação manual do usuário.
 */
function renderRichText(valor, { destaque = true } = {}) {
  if (!valor) return '';
  if (!HTML_TAG_RE.test(valor)) {
    return `<p>${destaque ? escComDestaque(valor) : esc(valor)}</p>`;
  }
  return sanitizeRichText(destaque ? destacarSemEscapar(valor) : valor);
}

/**
 * Converte texto multilinha em bullets HTML — cada linha que começa com
 * • ou - ou * vira <li>. Linhas vazias separam parágrafos.
 */
function textoParaBullets(texto) {
  if (!texto) return '';
  const linhas = texto.split('\n').map(l => l.trim()).filter(Boolean);
  const isBullet = (l) => /^[•\-*]\s/.test(l);
  if (linhas.every(isBullet)) {
    return '<ul>' + linhas.map(l => `<li>${escComDestaque(l.replace(/^[•\-*]\s+/, ''))}</li>`).join('') + '</ul>';
  }
  return '<p>' + linhas.map(l => escComDestaque(isBullet(l) ? '• ' + l.replace(/^[•\-*]\s+/, '') : l)).join('<br>') + '</p>';
}
 ...
 */
function renderHtml(proposta, _opts = {}) {
  const p = proposta;
  ...
  const css = `
    ...
    .clausula .texto-cl p, .escopo-item p { margin: 0; }
    .escopo-item { position: relative; padding-left: 18px; margin-bottom: 6px; }
    .escopo-item::before { content: '•'; position: absolute; left: 4px; }
    .escopo-item table, .clausula table { margin: 6px 0 6px 0; }
  ...
  const renderClausulas = (lista) => {
    ...
    return lista.map(c => `
      <div class="clausula">
        ${c.titulo ? `<div class="titulo-cl">${escComDestaque(c.titulo)}</div>` : ''}
        <div class="texto-cl">${renderRichText(c.texto)}</div>
      </div>
    `).join('');
  };
...
${renderRichText(p.saudacao || cfg.PADRAO.SAUDACAO)}

${(() => {
  // Apresentação global (definida em #/apresentacao) — comum a todas as propostas
  const apr = p._apresentacao || {};
  const logos = Array.isArray(p._caseLogos) ? p._caseLogos : [];
  let html = '';
  if (apr.apresentacao) {
    html += `<h2>Apresentação</h2>${textoParaBullets(apr.apresentacao)}`;
  }
  if (apr.casesSucesso || logos.length > 0) {
    html += `<h2>Cases de Sucesso Recentes</h2>`;
    if (apr.casesSucesso) html += textoParaBullets(apr.casesSucesso);
    if (logos.length > 0) {
      html += `<div class="case-logos-grid" style="display:flex;flex-wrap:wrap;gap:14px;align-items:center;justify-content:center;margin:10px 0;">`;
      for (const lg of logos) {
        // Em preview HTML, usa URL da API; quando convertido pra PDF via puppeteer (não usamos
        // mais), seria preciso embedar. Como agora PDF é via pdfkit, isso só importa pro preview.
        html += `<img src="/api/case-logos/${esc(lg.id)}/image" alt="${esc(lg.nome)}" style="max-width:130px;max-height:70px;object-fit:contain;">`;
      }
      html += `</div>`;
    }
  }
  if (apr.segurancaSaude) {
    html += `<h2>Segurança e Saúde</h2>${textoParaBullets(apr.segurancaSaude)}`;
  }
  return html;
})()}
...
${p.objetivo ? `
  <h2>Objetivo</h2>
  ${renderRichText(p.objetivo)}
` : ''}

<h2>Escopo</h2>
${inclusos.length === 0 ? '<p style="color:#888;font-style:italic;">— Não definido —</p>' : `
  <div class="escopo-lista">${inclusos.map(it => `<div class="escopo-item">${renderRichText(it.texto, { destaque: false })}</div>`).join('')}</div>
`}
...
${exclusoes.length ? `
  ...
  <div class="escopo-lista">${exclusoes.map(it => `<div class="escopo-item">${renderRichText(it.texto, { destaque: false })}</div>`).join('')}</div>
` : ''}
...
<p>${escComDestaque(p.condicoesPagamento || cfg.PADRAO.CONDICOES_PAGAMENTO)}</p>

${p.prazoExecucao ? `
  <h2>Prazo de Execução</h2>
  <p>${escComDestaque(p.prazoExecucao)}</p>
` : ''}
...
${p.observacoes ? `
  <h2>Observações</h2>
  ${renderRichText(p.observacoes, { destaque: false })}
` : ''}
