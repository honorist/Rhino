...
const { renderRichTextPdf } = require('./rich-text-to-pdfkit');

// Campo de texto rico "de verdade" (editor Jodit) vs. texto legado — mesma
// detecção usada em lib/proposta-html.js#renderRichText e lib/proposta-docx.js.
...
function _gerarPdfPrincipal(p) {
  return new Promise((resolve, reject) => {
    try {
      ...
      // Estratégia das 2 camadas:
      //  1. No evento pageAdded: desenha APENAS a imagem de fundo (não usa
      //     text(), então não dispara nova pageAdded → sem recursão).
      //     O conteúdo do _renderConteudo escreve POR CIMA naturalmente.
      //  2. No final, via bufferedPageRange + switchToPage: escreve a
      //     numeração POR CIMA do tudo (sem moveDown nem addPage, só text
      //     absoluto com lineBreak:false).
      const drawBackground = () => {
        ...
        try {
          ...
          console.warn('[proposta-pdf] bg falhou:', e.message);
        }
      };

      // Primeira página (não dispara pageAdded — é criada no construtor)
      drawBackground();
      ...
      // Camada de numeração: itera todas as páginas, escreve número por cima.
      // CRÍTICO: captura `total` ANTES do loop. Se um text() dentro do loop
      // disparar pageAdded (auto-page), `bufferedPageRange().count` cresce e
      // o loop fica eterno OU os números ficam errados (Página 1 de 3 na
      // página 4, etc.). Aqui congelamos o total no momento de captura.
      const range = doc.bufferedPageRange();
      const totalPages = range.count;
      for (let i = 0; i < totalPages; i++) {
        doc.switchToPage(range.start + i);
        try {
          // Salva estado, desenha numeração com posição/tamanho que NÃO causa
          // overflow (lineBreak:false + width definida + height limitada).
          doc.save();
          doc.font('Helvetica').fontSize(8).fillColor('#555')
             .text(`Página ${i + 1} de ${totalPages}`,
                   doc.page.width - 130, doc.page.height - 30,
                   { width: 80, align: 'right', lineBreak: false, height: 12 });
          doc.restore();
        } catch (_) { /* ignora — só estética */ }
      }
...
function _renderConteudo(doc, p) {
  ...
  // Anexos já carregados com binário — resolve <img src="/api/propostas/:id/anexos/:id">
  // embutida em campos de texto rico.
  const anexoMap = {};
  (p.anexos || []).forEach(a => { if (a.data) anexoMap[a.id] = { data: a.data, mimeType: a.mimeType }; });
  ...
  const saudacaoVal = p.saudacao || cfg.PADRAO.SAUDACAO;
  if (HTML_TAG_RE.test(saudacaoVal)) {
    renderRichTextPdf(doc, saudacaoVal, { anexoMap, destaque: true });
    doc.moveDown(0.2);
  } else {
    _paraDestaque(doc, saudacaoVal);
  }
  ...
  if (p.objetivo) {
    _h(doc, 'Objetivo');
    if (HTML_TAG_RE.test(p.objetivo)) renderRichTextPdf(doc, p.objetivo, { anexoMap, destaque: true });
    else _paraDestaque(doc, p.objetivo);
  }
  ...
  if (inclusos.length === 0) {
    ...
    inclusos.forEach(it => {
      ...
      if (HTML_TAG_RE.test(texto)) renderRichTextPdf(doc, texto, { anexoMap, destaque: true, align: 'left' });
      else _bullet(doc, texto);
    });
  }

  // Imagens ilustrativas (anexos secao=escopo) — DEPOIS do Escopo, antes das Exclusões
  ...
  _renderClausulas(doc, p.obrigacoesContratada, anexoMap);

  _h(doc, 'Obrigações da Contratante');
  _renderClausulas(doc, p.obrigacoesContratante, anexoMap);
  ...
  if (p.observacoes) {
    _h(doc, 'Observações');
    if (HTML_TAG_RE.test(p.observacoes)) renderRichTextPdf(doc, p.observacoes, { anexoMap, destaque: true });
    else _paraDestaque(doc, p.observacoes);
  }
...
function _renderClausulas(doc, lista, anexoMap) {
  ...
  lista.forEach(c => {
    ...
    const texto = c.texto || '';
    if (HTML_TAG_RE.test(texto)) renderRichTextPdf(doc, texto, { anexoMap, destaque: true });
    else _paraDestaque(doc, texto);
    doc.moveDown(0.2);
  });
}
