...
/**
 * Apaga linhas cujo `id` não está mais no array `currentIds`.
 *
 * FIX P0-1 (DB review): array vazio significava `DELETE FROM <table>` sem WHERE
 * — apagava TUDO. Se um JSON-source ficasse vazio por erro de leitura ou race
 * de migração, a tabela inteira era destruída (com CASCADE em filhos: saidas,
 * organograma_membros, rdos). Agora um array vazio é tratado como "no-op" para
 * evitar destruição catastrófica. Para limpar uma tabela intencionalmente, use
 * `db.query('DELETE FROM ...')` diretamente.
 *
 * @param {string} table   Nome da tabela (já validado pelos mappers).
 * @param {string[]} currentIds  IDs presentes no source-of-truth atual.
 */
async function deleteMissing(table, currentIds) {
  if (!Array.isArray(currentIds)) return;
  if (currentIds.length === 0) {
    // Guard explícito — não apaga toda a tabela quando o source vem vazio.
    // Loga para investigação (provavelmente bug ou JSON corrompido).
    console.warn(`[pg-sync] deleteMissing("${table}", []) skipped — array vazio. Use db.query() direto para truncar.`);
    return;
  }
