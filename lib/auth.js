 ...
 *
 * TODO (M-05 da security review): migrar para `bcrypt` (binding C++) — é 3-5×
 * mais rápido que `bcryptjs`, aumentando o custo defensivo contra brute force
 * offline sem aumentar latência de login.
 */
...
async function bootstrapAdmin() {
  ...
  if (password.length < 10 || /^(admin|senha|123|password|teste|test)/i.test(password)) {
    console.warn('[auth] ⚠️  ADMIN_PASSWORD fraca/padrão — troque por uma senha forte (>12 chars) e remova-a do ambiente após o primeiro login.');
  }
