...
/**
 * Checagem crua: o perfil do `req.user` tem exatamente a string `aba` em seu
 * array `abas`? Difere de `can()` de propósito — aqui `nivelAcessoId` nulo
 * libera (super admin sem perfil), mas `'admin'` NÃO é bypass: cai na consulta
 * do perfil. Preserva o comportamento do antigo `_temPermissao` inline do
 * server.js, usado por Cobrança e pelo workflow de Compras/Manutenções, onde as
 * `abas` carregam permissões de ação cruas (ex.: 'solicitacoes-compra:aprovar')
 * que não passam pelo mapa resource×route de `can()`.
 *
 * @param {{ user?: { nivelAcessoId?: string|null } }} req
 * @param {string} aba
 * @returns {Promise<boolean>}
 */
async function temAba(req, aba) {
  const nivelId = req.user?.nivelAcessoId;
  if (!nivelId) return true;
  const nivel = await repos.niveisAcesso.findById(nivelId);
  return !!(nivel?.abas || []).includes(aba);
}
