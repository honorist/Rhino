 ...
 *    E o avanço físico das atividades atingiu a meta (100%). Cada bloqueio
 *    vira uma linha em `pendencias` (texto pronto para exibir).
 *
 * O avanço físico NÃO é calculado aqui: vem de lib/avanco-fisico.js, a mesma
 * conta usada pelo Cronograma. Antes este arquivo fazia média simples enquanto
 * a tela de Cronograma ponderava por peso — a mesma obra exibia dois números
 * de "avanço físico" diferentes conforme a aba aberta.
 */

/** Avanço físico médio (%) mínimo para a obra ser considerada pronta. */
const { avancoFisicoPonderado } = require('./avanco-fisico');
...
function prontidao({ punchItens, atividades } = {}) {
  ...
  // ── Avanço físico (fonte única: lib/avanco-fisico.js) ──
  // Antes era média simples aqui e ponderada por peso no Cronograma, então a
  // MESMA obra exibia dois "avanço físico" diferentes conforme a aba. Agora as
  // duas leem a mesma conta: ponderada quando há peso, média simples quando os
  // pesos não foram preenchidos (`base` diz qual foi usada).
  const avanco = avancoFisicoPonderado(ativs);
  // `pct` é null quando não há atividade nenhuma; a saída pública mantém número
  // (a view já consome `execMedio`), e quem precisa distinguir usa `base`.
  const execMedio = avanco.pct === null ? 0 : avanco.pct;
  ...
  return {
    ...
    // `base` distingue "ponderado por peso" de "média simples (sem pesos)" e de
    // "sem cronograma" — sem isso a tela não tem como ser honesta sobre 0%.
    fisico: { execMedio, base: avanco.base },
    pronto,
    pendencias,
  };
}
