...
/**
 * Homem-hora trabalhado da obra, somando `totais.totalHomemHora` dos RDOs.
 * É a base de TF/TG — sem ela as duas taxas caem para 0.
 *
 * Estava dentro de handlers/ssma.js (`_hhtDoContrato`), colado ao I/O de buscar
 * os RDOs; virou função pura aqui porque é regra (steering §3) e porque o
 * Painel da obra precisa da mesma conta sobre RDOs que ele já carregou, sem
 * repetir a query.
 *
 * @param {object[]} [rdos]
 * @returns {number}
 */
function hhtDeRdos(rdos) {
  let hht = 0;
  for (const r of Array.isArray(rdos) ? rdos : []) {
    const t = r && r.totais;
