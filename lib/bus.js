...
const { podeReceberMutacao } = require('./bus-entity-routes');
 ...
 * @property {string[] | null} abas  Permissões do nível de acesso do usuário
 *   (null = sem nível atrelado = sem restrição). Usado por publish() pra não
 *   mandar mutação de entidade que este cliente não tem permissão de ver.
...
class Bus {
   ...
   * @param {{ userId?: string|null, userEmail?: string|null, abas?: string[]|null }} [opts]
   */
  attach(req, res, { userId, userEmail, abas } = {}) {
    res.writeHead(200, {
  ...
  /**
   * Transmite um evento de mutação — só pros clientes cujo nível de acesso
   * tem permissão na rota daquela entidade (lib/bus-entity-routes.js).
   * Presença/hello continuam broadcast puro: não são dado sensível.
   *
  ...
  publish(event) {
    ...
    for (const c of this.clients) {
      if (podeReceberMutacao(event.entity, c.abas)) this._send(c.res, 'mutation', event);
    }
  }
