...
const comprasHandlers = require('./handlers/compras'); // solicitações de compra: avaliar→aprovar→comprar→receber
...
const { ensureAlmoxarifadoCentral } = estoqueHandlers; // usado no startup; _resolveAlmoxId/_ajustarSaldo migraram p/ handlers/compras.js
...
// ============ Solicitações de Compra + Manutenção de Equipamentos ============
// → handlers/compras.js (fluxo avaliar→aprovar→comprar→receber, com Conta a
// Pagar e entrada de estoque + custo médio sob transação) e
// handlers/manutencoes.js (fluxo de manutenção de equipamentos). Registrados
// em registerOperacao. O custo médio ponderado é a regra pura lib/estoque-custo.
