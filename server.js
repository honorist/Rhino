...
function handlePostSaida(contractId, body, res) {
  try {
    ...
    // Validação: BM não pode ultrapassar valor do contrato
    ...
    // Numero sequencial do BM para este contrato (BM-001, BM-002...)
    const numeroBm = String(nfsContrato.length + 1).padStart(3, '0');
    const numeroNf = `BM-${numeroBm}`;

    // Cria a NF (BM) em Contas a Receber, status cadastrada (não emitida)
    const nf = {
      id: generateId('nf'),
      numero: numeroNf,
      contractId,
...
function handlePutSaida(id, body, res) {
  try {
    ...
    // Sincronizar com a NF vinculada (BM)
    let nfData = null;
    if (saida.nfId) {
...
function handleDeleteSaida(id, res) {
  try {
    ...
    let nfData = null;
    // Remove a NF (BM) vinculada se ainda não foi emitida
    if (saida && saida.nfId) {
      nfData = readData('notas_fiscais.json');
      const nfIdx = (nfData.notas_fiscais || []).findIndex(nf => nf.id === saida.nfId);
      if (nfIdx !== -1) {
