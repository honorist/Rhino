...
window.Auditoria = {
  ...
  _entityInfo(e) {
    const map = {
      ...
      'manutencoes.fotos': ['Foto de manutenção', 'a'],
      veiculos: ['Veículo', 'o'],
  ...
  _entityLabel(e) {
    return this._entityInfo(e).label;
  },
  ...
  _entityFriendlyName(entity, entityId) {
    ...
    const find = (arr, key) =>
      Array.isArray(arr) ? (arr.find((x) => x?.id === entityId) || {})[key] || '' : '';
    switch (entity) {
      case 'contracts':
      ...
      case 'propostas':
        return find(s.propostas, 'titulo') || find(s.propostas, 'nome');
      case 'clausulas':
        return find(s.clausulas, 'titulo') || find(s.clausulas, 'nome');
      case 'manutencoes':
        return find(s.manutencoes, 'equipamento');
      case 'manutencoes.fotos': {
        // entityId pode ser o ID da manutenção (em POSTs) ou da foto (em DELETEs)
        const mf = (s.manutencoes || []).find((x) => x?.id === entityId);
        return mf ? mf.equipamento || '' : '';
      }
      case 'solicitacoes-compra': {
        const sc = (s.solicitacoes_compra || []).find((x) => x?.id === entityId);
        return sc ? sc.justificativa || sc.descricao || '' : '';
      }
      case 'veiculos':
        return find(s.veiculos, 'placa') || find(s.veiculos, 'nome');
      case 'users': {
  ...
  _groupByDay(rows) {
    ...
    for (const r of rows) {
      ...
      const key = d.getTime();
      if (!cur || cur.key !== key) {
        cur = { key, label: this._dayLabel(d), rows: [] };
        groups.push(cur);
      }
      cur.rows.push(r);
    }
  ...
  _eventTarget(r) {
    ...
    const storeNome = this._entityFriendlyName(r.entity, r.entityId);
    // rejeita entityLabel que seja apenas dígitos (SERIAL do banco como "7", "12")
    const dbLabel = r.entityLabel && !/^\d+$/.test(r.entityLabel) ? r.entityLabel : '';
    const nome = storeNome || dbLabel;
    const nameHtml = nome
      ? ` <strong class="audit-name">"${escapeHtml(nome)}"</strong>`
      : r.entityId && r.action !== 'create'
        ? ' <span class="audit-removed">(removido)</span>'
        : '';

    if (r.action === 'passagem') return nome ? `para <strong>"${escapeHtml(nome)}"</strong>` : '';
    if (r.action === 'gerar' && r.entity === 'folha-pagamento')
      return 'a <strong>folha de pagamento</strong> do mês';
    if (r.action === 'limpar' && r.entity === 'folha-pagamento')
      return 'a <strong>folha de pagamento</strong>';
  ...
  _fieldMeta(key) {
    const M = {
      execPct: ['Execução', 'percent'],
      pesoPct: ['Peso', 'percent'],
      aderencia: ['Aderência', 'percent'],
      retencaoPercent: ['Retenção', 'percent'],
      participacao: ['Participação', 'percent'],
      percentual: ['Percentual', 'percent'],
      value: ['Valor', 'money'],
      valor: ['Valor', 'money'],
      valorTotal: ['Valor total', 'money'],
      valorPago: ['Valor pago', 'money'],
      valorDelta: ['Δ Valor', 'money'],
      salario: ['Salário', 'money'],
      custo: ['Custo', 'money'],
      custoEstimado: ['Custo estimado', 'money'],
      custoPlan: ['Custo planejado', 'money'],
      preco: ['Preço', 'money'],
      saldo: ['Saldo', 'money'],
      startDate: ['Início', 'date'],
      endDate: ['Término', 'date'],
      tendencyDate: ['Tendência', 'date'],
      date: ['Data', 'date'],
      dataFimPlan: ['Data fim', 'date'],
      dataInicioPlan: ['Data início', 'date'],
      dataVencimento: ['Vencimento', 'date'],
      dataEmissao: ['Emissão', 'date'],
      dataPagamento: ['Pagamento', 'date'],
      dataAdmissao: ['Admissão', 'date'],
      dataNascimento: ['Nascimento', 'date'],
      dataRetorno: ['Retorno', 'date'],
  ...
  _updateSummaryHtml(r) {
    ...
    const it = diffs
      .map((d) => {
        const m = this._fieldMeta(d.key);
        return { label: m.label, type: m.type, old: d.before, new: d.after };
      })
      // remove diffs onde old e new têm a mesma representação (ex: [1 item]→[1 item])
      .filter((x) => this._fmtTyped(x.old, x.type) !== this._fmtTyped(x.new, x.type));
    if (!it.length) return null;
    const lbl = (x) => `<strong>${escapeHtml(x.label)}</strong>`;
    const pair = (x) =>
      `<span class="audit-diff">${escapeHtml(this._fmtTyped(x.old, x.type))}→${escapeHtml(this._fmtTyped(x.new, x.type))}</span>`;
    if (it.length === 1) return `editou ${lbl(it[0])} ${pair(it[0])}`;
    return `editou ${it.map(lbl).join(', ')}`;
  },
  ...
  async render() {
    ...
    try {
      ...
      console.warn('[Auditoria] Store.loadAll falhou — nomes podem virar IDs:', e?.message || e);
    }
    await this._fetch();
    this._draw();
  },
  ...
  _renderFeed(rows) {
    ...
    return `<div class="audit-feed">${groups
      .map(
        (g) => `
    ...
    if (r.action === 'update') {
      ...
      if (sum) {
        ...
        whatHtml =
          sum +
          (nome
            ? ` <span class="audit-ctx">· ${escapeHtml(info.label.toLowerCase())} "${escapeHtml(nome)}"</span>`
            : '');
      } else {
  ...
  _wire() {
    ...
    ['fAuditEntity', 'fAuditAction', 'fAuditFrom', 'fAuditTo'].forEach((id) => {
      const el = $(id);
      if (el) el.addEventListener('change', onAdv);
    });
  ...
  _fmtVal(v, key = '') {
    ...
    if (typeof v === 'number') {
      ...
      const isCurrency =
        /^(value|valor|valorpago|preco|salario|saldo|total|montante|custo|retencao)$/i.test(key);
      if (isCurrency)
