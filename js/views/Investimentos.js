window.Investimentos = {
  ...
  async render() {
    ...
    try {
            ...
            ? window.UIKit.kpiGrid([
                {
                  ...
                  value: Store.formatBRL(totalGeral),
                  color: 'var(--color-primary)',
                },
                {
                  icon: '👥',
                  label: 'Aportes dos Sócios',
                  value: Store.formatBRL(totalSocios),
                  color: 'var(--color-info)',
                },
                {
                  icon: '💰',
                  label: 'Via Caixa da Empresa',
                  value: Store.formatBRL(totalCaixa),
                ...
                <tbody>
                  ${Store.state.socios
                    .map((socio) => {
                      ...
                      const esperado =
                        totalSocios > 0 ? (totalSocios * num(socio.participacao)) / 100 : 0;
      ...
      document.querySelectorAll('.btn-excluir-aporte').forEach((btn) => {
        btn.addEventListener('click', (e) => {
          e.stopPropagation();
          this.deleteAporte(e.target.dataset.id);
        });
      });
      ...
      console.error(e);
      app.innerHTML =
        '<div class="card"><p class="text-danger">Erro ao carregar investimentos. Tente novamente.</p></div>';
    }
  },
  ...
  showDetail(id) {
    ...
    const contract = ap.contractId ? Store.getContractById(ap.contractId) : null;
    const baseItem = ap.baseItemId
      ? (Store.state.base || []).find((b) => b.id === ap.baseItemId)
      : null;
    const caixaEntry = ap.caixaEntryId
      ? (Store.state.caixa || []).find((e) => e.id === ap.caixaEntryId)
      : null;
    ...
    const origemLabel =
      ap.origem === 'socio'
        ? '👤 Sócio'
        : ap.origem === 'caixa_empresa'
          ? '💼 Caixa da empresa'
          : escapeHtml(ap.origem || '—');
    const destinoLabel =
      ap.destino === 'base'
        ? '⚙️ BASE'
        : ap.destino === 'contrato'
          ? '📋 Contrato'
          : escapeHtml(ap.destino || '—');
    ...
    document.getElementById('btnDetDel').addEventListener('click', () => {
      close();
      this.deleteAporte(id);
    });
  },
  ...
  showModal() {
    ...
    document.getElementById('btnSalvar').addEventListener('click', async () => {
      ...
      if (data.origem === 'socio' && !data.socioId) {
        window.showToast('Selecione o sócio', 'error');
        return;
      }
      if (data.destino === 'contrato' && !data.contractId) {
        window.showToast('Selecione o contrato de destino', 'error');
        return;
      }
      ...
      <div class="modal-overlay" id="modalOverlay">
        <div class="modal" style="width:700px;">
          ...
          <div class="modal-content">
            ...
            ${
              aportes.length === 0
                ? `<p class="text-muted">Nenhum aporte registrado para este sócio</p>`
                : `
              <div class="table-wrap">
                <table>
