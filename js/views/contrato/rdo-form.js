...
(function () {
  if (!window.ContratoDetail) {
    console.error('[contrato/rdo-form] requires ContratoDetail core');
    return;
  }
  Object.assign(window.ContratoDetail, {
    _renderRdoModal(contractId, rdoOriginal) {
      ...
      const tabs = [
        ...
        {
          k: 'fotos',
          l: `Fotos${rdoOriginal ? ' (' + (rdoOriginal.fotos || []).length + ')' : ''}`,
        },
      ];
    ...
    _renderRdoTab(tab, rdoOriginal) {
      ...
      switch (tab) {
        case 'cabecalho':
          return this._rdoTabCabecalho(d);
        case 'tempo':
          return this._rdoTabTempo(d);
        case 'mo':
        ...
        case 'atividades':
          return this._rdoTabAtividades(d);
        case 'seguranca':
          return this._rdoTabSeguranca(d);
        case 'fiscalizacao':
          return this._rdoTabFiscalizacao(d);
        case 'fotos':
          return this._rdoTabFotos(d, rdoOriginal);
        default:
          return '';
      }
    },
    ...
    _rdoMoRow(cat, entry, idx, opcoesCargo) {
      const options = opcoesCargo
        .map((c) => `<option ${entry.cargo === c ? 'selected' : ''}>${c}</option>`)
        .join('');
      return `
    ...
    _rdoTabAtividades(d) {
      ...
      const membrosDatalist = `<datalist id="rdo-membros-list">${recursos
        .map((r) => `<option value="${escapeHtml(r.nome || '')}">`)
        .join('')}</datalist>`;
      return `
      ${membrosDatalist}
    ...
    _rdoEquipeDuracaoHoras(eq) {
      const [hi, mi] = String(eq.horaInicio || '')
        .split(':')
        .map(Number);
      const [hf, mf] = String(eq.horaFim || '')
        .split(':')
        .map(Number);
    ...
    _attachRdoModalListeners(contractId, rdoOriginal) {
      ...
      document.getElementById('btnSaveRdo').addEventListener('click', async () => {
        try {
          ...
          for (const [ai, atv] of (this._rdoData.atividades || []).entries()) {
            for (const eq of atv.equipes || []) {
              if (eq.horaInicio && eq.horaFim && this._rdoEquipeDuracaoHoras(eq) <= 0) {
                showToast(
                  `Atividade ${ai + 1}, equipe "${eq.nome || 'sem nome'}": término deve ser após o início.`,
                  'warning'
                );
                return;
              }
          ...
          t.homensHora = ['moi', 'mod', 'terc'].reduce(
            (s, k) =>
              s +
              (this._rdoData[k] || []).reduce((acc, x) => acc + (+x.qtd || 0) * (+x.horas || 0), 0),
            0
          );
          t.equipamentoHora = (this._rdoData.equipamentos || []).reduce(
            (acc, x) => acc + (+x.qtd || 0) * (+x.horas || 0),
            0
          );
    ...
    _bindRdoInputs(contractId, rdoOriginal) {
        ...
        const handler = () => {
          ...
          if (path === 'data') {
            ...
            if (contract) {
              ...
              const atrasoDias =
                contract.tendencyDate && contract.endDate
                  ? Math.max(
                      0,
                      Math.round(
                        (new Date(contract.tendencyDate) - new Date(contract.endDate)) / 86400000
                      )
                    )
                  : 0;
              this._rdoData.prazo = {
      ...
      overlay.querySelectorAll('[data-rdo-terc]').forEach((el) => {
        el.addEventListener('input', () => {
          ...
          if (!this._rdoData.terc[idx])
            this._rdoData.terc[idx] = { empresa: '', cargo: '', qtd: 0, horas: 9 };
          this._rdoData.terc[idx][key] =
      ...
      overlay.querySelectorAll('[data-rdo-eqp]').forEach((el) => {
        el.addEventListener('input', () => {
          ...
          if (!this._rdoData.equipamentos[idx])
            this._rdoData.equipamentos[idx] = { tipo: '', qtd: 0, horas: 9 };
          this._rdoData.equipamentos[idx][key] =
            el.type === 'number' ? parseFloat(el.value) || 0 : el.value;
        });
      });
      ...
      const hhArr = () => {
        ...
        if (!Array.isArray(this._rdoData.passarelli.detalhamentoHorario))
          this._rdoData.passarelli.detalhamentoHorario = [];
      ...
      const hhRecalcLinha = (idx) => {
        ...
        if (row) {
          const q = row.querySelector('.rdo-hh-qtd');
          if (q) q.textContent = c.qtdHoras.toFixed(2);
          const t = row.querySelector('.rdo-hh-total');
          if (t) t.textContent = c.horaTotalHH.toFixed(2);
        }
      ...
      overlay.querySelectorAll('[data-rdo-hh]').forEach((el) => {
        el.addEventListener('input', () => {
          ...
          const arr = hhArr();
          if (!arr[idx])
            arr[idx] = { funcao: '', horaIni: '', horaFim: '', refeicaoMin: 0, efetivo: 0 };
          arr[idx][key] = el.type === 'number' ? parseFloat(el.value) || 0 : el.value;
          hhRecalcLinha(idx);
        });
      });
      ...
      overlay.querySelectorAll('[data-rdo-atv]').forEach((el) => {
        el.addEventListener('input', () => {
          ...
          if (!this._rdoData.atividades[idx])
            this._rdoData.atividades[idx] = {
              area: '',
              descricao: '',
              pctConcluida: 0,
              ocorrencias: '',
            };
          this._rdoData.atividades[idx][key] =
            el.type === 'number' ? parseFloat(el.value) || 0 : el.value;
        });
      });
      ...
      overlay.querySelectorAll('[data-rdo-eq]').forEach((el) => {
        el.addEventListener('input', () => {
          ...
          if (!atv.equipes[parseInt(eStr)])
            atv.equipes[parseInt(eStr)] = { nome: '', horaInicio: '', horaFim: '', membros: [] };
    ...
    async deleteRdo(contractId, rdoId) {
      ...
      if (
        !confirm(
          `Excluir RDO #${rdo.numero} de ${rdo.data}? Todas as fotos também serão removidas.`
        )
      )
        return;
      try {
