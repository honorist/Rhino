...
window.Comparativo = {
  ...
  _calcMetrics(c) {
    ...
    const baseAlocs = (Store.state.base || []).filter((b) =>
      (b.contracts || []).some((a) => a.contractId === id)
    );
    const totalBase = baseAlocs.reduce(
      (s, b) => s + ((b.contracts || []).find((a) => a.contractId === id)?.value || 0),
      0
    );
    const compras = (Store.state.caixa || []).filter(
      (e) => e.contractId === id && e.type === 'saida' && e.category !== 'passagem'
    );
    const totalCompras = compras.reduce((s, e) => s + (parseFloat(e.value) || 0), 0);
    const passagens = (Store.state.caixa || []).filter(
      (e) => e.contractId === id && e.category === 'passagem' && e.type === 'saida'
    );
    ...
    const equipeAtual = (Store.state.recursos || []).filter(
      (r) => r.status === 'funcionario' && r.alocacaoAtual?.contractId === id
    ).length;
    ...
    const rdosUltimos30 = rdos.filter(
      (r) => r.data && new Date(r.data + 'T12:00:00') >= limite
    ).length;
  ...
  _draw() {
    ...
    else if (this._filtroStatus === 'concluidos')
      contratos = contratos.filter((c) => c.status === 'concluido');

    const metrics = contratos.map((c) => this._calcMetrics(c));
    metrics.sort((a, b) => {
      const va = a[this._sortBy],
        vb = b[this._sortBy];
    ...
    const corPct = (p, ref) =>
      p >= ref ? 'var(--color-success)' : p >= 0 ? '#F59E0B' : 'var(--color-danger)';
      ...
      ${
        window.UIKit?.pageHeader
          ? window.UIKit.pageHeader({
              ...
              subtitle:
                'Ranking por margem, atraso, execução e mais — clique nas colunas para ordenar',
              actions:
          ...
          ? window.UIKit.kpiGrid([
              {
                ...
                hint: `${metrics.length} contrato${metrics.length !== 1 ? 's' : ''}`,
              },
              {
                label: 'Total medido (BMs)',
                value: fmt(totalMedido),
              ...
              { label: 'Total custo', value: fmt(totalCusto), color: 'var(--color-warning)' },
              {
                ...
                color: corPct(pctMargemAgregado, 20),
                hint: `${pctMargemAgregado.toFixed(1)}% (meta ≥20%)`,
              },
            ])
          : ''
      }
          ...
          ? window.UIKit.chips(
              [
                { value: 'ativos', label: 'Ativos', active: this._filtroStatus === 'ativos' },
                {
                  value: 'concluidos',
                  label: 'Concluídos',
                  active: this._filtroStatus === 'concluidos',
                },
    ...
    document.querySelectorAll('[data-chips="comp-status"] .rh-chip').forEach((b) => {
      b.addEventListener('click', () => {
        this._filtroStatus = b.dataset.value;
        this._draw();
      });
    });
    document.querySelectorAll('button[data-filtro]').forEach((b) => {
      b.addEventListener('click', () => {
        this._filtroStatus = b.dataset.filtro;
        this._draw();
      });
    });
    document.querySelectorAll('.row-comp').forEach((tr) => {
      tr.addEventListener('click', () => {
        location.hash = `#/contratos/${tr.dataset.id}`;
      });
    });
  },
};
