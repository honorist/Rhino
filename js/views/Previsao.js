...
window.Previsao = {
  ...
  async render() {
    ...
    try {
      const res = await fetch(`/api/dashboard?projDays=${this._days}`, {
  ...
  async _renderPage(app, data) {
    ...
    const kpisHtml = window.UIKit?.kpiGrid
      ? window.UIKit.kpiGrid([
          {
            label: 'Saldo Atual',
            value: fmt(caixaBalance),
            color: caixaBalance >= 0 ? 'var(--color-success)' : 'var(--color-danger)',
          },
          {
            label: `Saldo Projetado (${this._days}d)`,
            value: fmt(saldoFinal),
            color: saldoFinal >= 0 ? 'var(--color-success)' : 'var(--color-danger)',
          },
          {
            label: 'Mínimo Projetado',
            value: fmt(minimoSaldo),
            color: minimoSaldo >= 0 ? 'var(--color-success)' : 'var(--color-danger)',
            hint: temNegativo ? '⚠️ Saldo negativo previsto' : '',
          },
          {
            label: 'CP Pendentes',
            value: fmt(contasPagarStatus.totalPendente || 0),
            color: 'var(--color-danger)',
            hint: `${contasPagarStatus.pendentes || 0} contas`,
          },
        ])
      : '';
  ...
  async _renderChart(points, saldoAtual) {
    ...
    const labels = [{ data: 'Hoje', saldo: saldoAtual }, ...points].map((p) => {
      ...
      return new Date(p.data + 'T12:00:00').toLocaleDateString('pt-BR', {
        day: '2-digit',
        month: 'short',
      });
    });
