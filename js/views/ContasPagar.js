window.ContasPagar = {
  ...
  async render() {
    ...
    try {
      ...
      fetch('/api/contas-pagar/processar-recorrencias', { method: 'POST' }).catch((e) =>
        console.warn('[ContasPagar] processar-recorrencias falhou:', e?.message || e)
      );
      ...
      const proximasVencer = pendentes.filter(
        (c) => c.dataVencimento && c.dataVencimento >= hojeStr && c.dataVencimento <= em7Str
      );
      ...
      const proximasTimeline = pendentes
        .filter((c) => {
          const diff = c.dataVencimento
            ? Math.floor((new Date(c.dataVencimento) - new Date()) / 86400000)
            : null;
      ...
      const kpisHtml = window.UIKit?.kpiGrid
        ? window.UIKit.kpiGrid([
            {
              label: 'A pagar',
              value: Store.formatBRL(totalPendente),
      ...
      const html = `
        ...
        ${
          window.UIKit?.chips
            ? `<div style="margin-bottom:var(--sp-md);">${window.UIKit.chips(
                [
                  {
                    value: 'pendente',
                    label: '⏳ Pendentes',
                    count: pendentes.length,
                    active: this.filtroStatus === 'pendente',
                  },
                  {
                    value: 'pago',
                    label: '✅ Pagas',
                    count: pagas.length,
                    active: this.filtroStatus === 'pago',
                  },
                  {
                    value: 'todos',
                    label: '📋 Todas',
                    count: contas.length,
                    active: this.filtroStatus === 'todos',
                  },
                ],
                { name: 'cp-status' }
              )}</div>`
                        ...
                        .map((c) => {
                          ...
                          const vencCor = vencida
                            ...
                            : proxima
                              ...
                              : 'var(--color-text)';
                          const diasLabel =
                            c.dataVencimento && c.status === 'pendente'
                              ? (() => {
                                  const dias = Math.floor(
                                    (new Date(c.dataVencimento) - new Date()) / 86400000
                                  );
                                  return dias < 0
                                    ? `<div style="font-size:15px;color:var(--color-danger);font-weight:700;">${Math.abs(dias)}d vencida</div>`
                                    : dias === 0
                                      ? `<div style="font-size:15px;color:var(--color-warning);font-weight:700;">vence hoje</div>`
      ...
      document.querySelectorAll('.btn-pagar').forEach((b) =>
        b.addEventListener('click', (e) => {
          e.stopPropagation();
          this.showModalPagar(e.target.dataset.id);
        })
      );
      document.querySelectorAll('.btn-estornar').forEach((b) =>
        b.addEventListener('click', (e) => {
          e.stopPropagation();
          this.estornar(e.target.dataset.id);
        })
      );
      document.querySelectorAll('.btn-editar-cp').forEach((b) =>
        b.addEventListener('click', (e) => {
          e.stopPropagation();
      ...
      document.querySelectorAll('.btn-excluir-cp').forEach((b) =>
        b.addEventListener('click', (e) => {
          e.stopPropagation();
          this.excluir(e.target.dataset.id);
        })
      );
  ...
  showDetail(id) {
    ...
    const fornecedor = c.fornecedorId
      ? (Store.state.fornecedores || []).find((f) => f.id === c.fornecedorId)
      : null;
  ...
  showModal(conta) {
        ...
        <div class="modal" style="width:580px;">
          ...
          <form id="formConta" class="modal-content">
            <div class="form-group">
              ...
              <input class="form-control" name="descricao" id="inputDescricao" value="${escapeHtml(conta?.descricao || '')}" required placeholder="Ex: Material elétrico, Serviço de transporte...">
            </div>
            <div class="form-row">
              <div class="form-group">
                <label class="form-label">Número da NF</label>
                <input class="form-control" name="numeroNF" value="${escapeHtml(conta?.numeroNF || '')}" placeholder="Ex: 001234">
              </div>
              <div class="form-group">
    ...
    if (chkRec && grpPer) {
      chkRec.addEventListener('change', () => {
        grpPer.style.display = chkRec.checked ? '' : 'none';
      });
    }
    ...
    if (btnAiCls) {
      btnAiCls.addEventListener('click', async () => {
        ...
        if (!desc) {
          window.showToast('Preencha a descrição primeiro', 'warn');
          return;
        }
        ...
        btnAiCls.disabled = true;
        btnAiCls.textContent = '⏳ Classificando…';
        try {
          const r = await fetch('/api/ai/classify-expense', {
            ...
            body: JSON.stringify({
              descricao: desc,
              valor: window.BRLInput?.parse(valorEl?.value) || 0,
              fornecedor: fornecNome,
            }),
            credentials: 'same-origin',
          });
          if (!r.ok) { const body = await r.text(); throw new Error(body && body.length < 120 ? body : 'Erro na classificação'); }
          ...
          btnAiCls.disabled = false;
          btnAiCls.textContent = '🤖 Classificar com IA';
        }
      });
    }
    ...
    document.getElementById('btnSalvar').addEventListener('click', async () => {
      ...
      if (!data.descricao?.trim()) {
        window.showToast('Descrição é obrigatória', 'error');
        return;
      }
      if (!data.dataVencimento) {
        window.showToast('Data de vencimento é obrigatória', 'error');
        return;
      }
  ...
  showModalPagar(id) {
          ...
          <div class="modal-content">
            ...
            <div class="form-group">
              ...
              <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:8px;" id="formasPagamento">
                ${['PIX', 'Boleto', 'Cartão', 'Transferência', 'Dinheiro', 'Cheque']
                  .map(
                    (f) => `
    ...
    document.getElementById('btnConfirmarPagamento').addEventListener('click', async () => {
      ...
      if (!dataPagamento) {
        window.showToast('Informe a data do pagamento', 'error');
        return;
      }
      try {
  ...
  async excluir(id) {
    if (!confirm('Excluir esta conta? Se estiver paga, a saída no caixa também será removida.'))
      return;
    try {
