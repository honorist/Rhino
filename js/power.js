...
(function () {
  ...
  // ───────────────────────────────────────────────
  // M6 · Notification bell (in-app)
  // Coleta sinais reais: vencimentos, RDOs sem lançamento, etc.
  // Roda quando Store está pronto.
  // ───────────────────────────────────────────────
  function buildNotifications() {
    ...
    // Contas a pagar vencidas / vencendo
    (S.contas_pagar || []).forEach((c) => {
      if (c.status !== 'pendente') return;
      const v = c.dataVencimento || c.data_vencimento;
      if (!v) return;
      if (v < todayStr) items.push({ icon: '🔴', label: `Conta vencida: ${c.descricao || c.fornecedor || 'sem descrição'}`, hash: '#/contas-pagar', priority: 1 });
      else if (v <= in7Str) items.push({ icon: '🟡', label: `Vence em ${Math.ceil((new Date(v) - today) / 86400000)}d: ${c.descricao || c.fornecedor || ''}`, hash: '#/contas-pagar', priority: 2 });
    });
    // Notas fiscais a receber atrasadas
    (S.notas_fiscais || S.notasFiscais || []).forEach((n) => {
      if (!n.emitida) return;
      if (n.caixaEntryId || n.caixa_entry_id) return;
      const v = n.dataVencimento || n.data_vencimento;
      if (v && v < todayStr) items.push({ icon: '⚠️', label: `NF atrasada: ${n.numero || n.descricao || ''}`, hash: '#/notas-fiscais', priority: 1 });
    });
    return items.sort((a, b) => a.priority - b.priority).slice(0, 25);
  }

  function renderBell() {
    let bell = document.querySelector('.rh-bell');
    if (!bell) {
      bell = document.createElement('button');
      bell.className = 'rh-bell theme-toggle-btn';
      bell.setAttribute('title', 'Notificações');
      bell.setAttribute('aria-label', 'Notificações');
      bell.innerHTML = '🔔<span class="rh-bell__count" hidden>0</span>';
      bell.addEventListener('click', toggleBellPanel);
      // Anexa no footer da sidebar se possível
      const footer = document.querySelector('.sidebar-footer');
      if (footer) footer.insertBefore(bell, footer.firstChild);
      else document.body.appendChild(bell);
    }
    const items = buildNotifications();
    const count = items.length;
    const badge = bell.querySelector('.rh-bell__count');
    if (count > 0) { badge.hidden = false; badge.textContent = count; }
    else { badge.hidden = true; }
    bell._items = items;
  }
  function toggleBellPanel() {
    let panel = document.querySelector('.rh-bell-panel');
    if (panel) { panel.remove(); return; }
    const bell = document.querySelector('.rh-bell');
    const items = (bell && bell._items) || [];
    panel = document.createElement('div');
    panel.className = 'rh-bell-panel';
    panel.innerHTML = items.length
      ? items.map((it) => `<a class="rh-bell-item" href="${it.hash}"><span>${it.icon}</span><span>${it.label}</span></a>`).join('')
      : '<div class="rh-bell-empty">Sem notificações 🎉</div>';
    document.body.appendChild(panel);
    setTimeout(() => {
      document.addEventListener('click', function once(e) {
        if (!panel.contains(e.target) && !e.target.closest('.rh-bell')) {
          panel.remove();
  ...
  RU.refreshBell = renderBell;
  // Render quando o app re-renderiza a sidebar (hashchange é proxy bom)
  window.addEventListener('hashchange', () => setTimeout(renderBell, 100));
