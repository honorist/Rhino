// ========================================================================
// apiMock.js — versão browser do server.js (persistência em localStorage)
// Gerado a partir de server.js preservando os handlers originais.
// Os shims abaixo permitem que os handlers rodem inalterados no navegador.
// ========================================================================
...
(function () {
  ...
  const DEFAULTS = {
    ...
    'tipos_base.json':    { tipos: [] },
    'contas_pagar.json':  { contas: [] },
    'notas_fiscais.json': { notas_fiscais: [] },
    'recursos.json':      { recursos: [] },
    'doc_templates.json': { templates: [] },
    'niveis_acesso.json': { niveis: [] }
  };
  ...
  // MockRes: substitui o res HTTP — captura status e body
  function makeRes() {
    return {
      _status: 200,
      _body: null,
      ...
      end(body) { this._body = body == null ? '' : body; return this; }
    };
  }

  // Stubs para evitar ReferenceError em handlers que mencionam fs/path
  const fs   = { existsSync: () => false, mkdirSync: () => {}, rmSync: () => {}, writeFileSync: () => {}, unlinkSync: () => {} };
  const path = { join: (...parts) => parts.filter(Boolean).join('/') };
  const __dirname = '';
...
function handlePostRdoFoto(contractId, rdoId, body, res) {
  try {
    ...
    const fotosInput = Array.isArray(body.fotos) ? body.fotos : [];
    ...
    const adicionadas = [];
    for (const arq of fotosInput) {
      if (!arq.dataUrl) continue;
      // Rough size check: base64 inflates ~4/3
      if (arq.dataUrl.length * 0.75 > FOTO_MAX_BYTES) continue;
      const fotoId = generateId('foto');
      const ext = ((arq.filename || '').match(/\.[a-z0-9]+$/i) || ['.jpg'])[0].toLowerCase();
      const fotoObj = {
        id: fotoId,
        filename: fotoId + ext,
        legenda: arq.legenda || legenda,
        url: arq.dataUrl,
        createdAt: new Date().toISOString()
      };
...
function routeRequest(pathname, method, body, res, parsedUrl) {
  ...
  if (pathname.match(/^\/api\/contracts\/[^/]+\/rdos\/[^/]+\/fotos$/) && method === 'POST') {
    const parts = pathname.split('/');
    return handlePostRdoFoto(parts[3], parts[5], body || {}, res);
  }
  ...
  res.writeHead(404, { 'Content-Type': 'application/json' });
  res.end(JSON.stringify({ error: 'Endpoint não encontrado (modo local)' }));
}
...
// ========================================================================
// Wrapper: expõe handleApiRequest e intercepta window.fetch
// ========================================================================

  async function handleApiRequest(method, urlStr, bodyObj) {
    ...
    // Rota de upload RDO é tratada com body já convertido para { fotos: [...] }
    const isRdoFotoUpload = method === 'POST' && /^\/api\/contracts\/[^/]+\/rdos\/[^/]+\/fotos$/.test(pathname);
    if (isRdoFotoUpload) {
      const parts = pathname.split('/');
      handlePostRdoFoto(parts[3], parts[5], bodyObj || {}, res);
    } else {
      try {
        routeRequest(pathname, method, bodyObj, res, parsedUrl);
      } catch (e) {
        res._status = 500;
        res._body = JSON.stringify({ error: e.message });
      }
    }

    // Aguarda eventuais setTimeouts síncronos resolverem
    await Promise.resolve();

    return new Response(res._body == null ? '' : res._body, {
  ...
  async function formDataToJson(formData) {
    ...
    for (const [key, value] of formData.entries()) {
      if (value instanceof File || value instanceof Blob) {
        ...
        fotos.push({ filename: value.name || 'foto', dataUrl });
      } else {
        result[key] = value;
      }
    }
    if (fotos.length > 0) result.fotos = fotos;
    return result;
  }

  // Intercepta fetch — só para /api/*
  const _origFetch = window.fetch.bind(window);
  window.fetch = async function (input, init) {
    const urlStr = typeof input === 'string' ? input : (input && input.url) || '';
    const apiMatch = urlStr.startsWith('/api/') || /^https?:\/\/[^/]+\/api\//.test(urlStr);
    if (!apiMatch) return _origFetch(input, init);
    ...
    return handleApiRequest(method, urlStr, body);
  };

  // Seed mínimo: níveis de acesso e tipos base para primeira abertura
  (function seed() {
    if (!localStorage.getItem(STORAGE_PREFIX + 'tipos_base.json')) {
      writeData('tipos_base.json', {
        tipos: [
          { id: 'tpb_sys_fixo',      key: 'fixo',     label: 'Fixo',     icon: '🏢', cor: '#718096', sistema: true, createdAt: new Date().toISOString() },
          { id: 'tpb_sys_variavel',  key: 'variavel', label: 'Variável', icon: '📊', cor: '#718096', sistema: true, createdAt: new Date().toISOString() },
          { id: 'tpb_sys_outros',    key: 'outros',   label: 'Outros',   icon: '📦', cor: '#718096', sistema: true, createdAt: new Date().toISOString() }
        ]
      });
    }
