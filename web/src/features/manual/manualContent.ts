/**
 * Conteúdo do Manual do Usuário — porte literal das strings HTML de
 * js/views/Manual.js (`_conteudo()` + helpers `_flow*`/`_journey*`/`_er*`).
 * Mantido como HTML porque o vanilla já usa `innerHTML` e a renderização do
 * Mermaid depende de blocos `<pre class="mermaid">` no DOM.
 */
...
/** Seção da navegação lateral do Manual. */
...
export const SECOES: SecaoManual[] = [
...
export const CONTEUDOS: Record<string, string> = {
  inicio: `
